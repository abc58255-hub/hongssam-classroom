...
function getSyllabusData() {
  try {
    ...
    // 반 설정 없으면 parseRosterAndClasses 활용
    if (classes.length === 0) {
      try {
        const r = parseRosterAndClasses(ss);
        classes = r.classList || [];
        ...
        if (rosterSheet && rosterSheet.getLastRow() >= 2) {
          const rData = rosterSheet.getRange(2,1,rosterSheet.getLastRow()-1,3).getValues();
          const clsSet = {};
          rData.forEach(function(r){
            const id = String(r[1]||'').trim();
            if (id.length >= 2) clsSet[id.substring(0,1)+'학년 '+id.substring(1,2)+'반'] = true;
          });
          classes = Object.keys(clsSet).sort();
        }
      }
    }
...
function analyzeTaskText(text, fileAttachment) {
  try {
    ...
    const userPrompt = '오늘 날짜: ' + today + '\n\n'
      ...
      + '- project: 내용에 맞는 분류 (학교업무/연수/학급운영/행사/제출/회의/기타 중 하나)\n'
      + '- tags: 관련 키워드 배열 (최대 3개, 한글)\n'
      + '- todoList: 내가 해야 할 구체적 행동 목록 배열 (동사로 시작, 최대 5개)\n'
      + '- considerations: 주의/고려해야 할 사항 배열 (최대 3개)\n'
      + '- docActions: 문서에서 해야 할 작업 배열 (서명, 도장, 결재 등, 없으면 빈 배열)\n'
      + '- checklist: 세부 단계가 있으면 배열, 없으면 빈 배열\n'
      ...
      + '      "time": "HH:mm 또는 null",\n'
      + '      "detail": "구체적으로 무엇을 해야 하는지",\n'
      + '      "priority": 1~5,\n'
      + '      "project": "분류명",\n'
      + '      "tags": ["태그1", "태그2"],\n'
      + '      "checklist": ["세부단계1", "세부단계2"],\n'
